...
def index():
    ...
    #return apology("TODO")
    stocks = db.execute("SELECT stock, price, SUM(shares) AS totals FROM purchases WHERE user_id = :user_id GROUP BY stock", user_id=session.get("user_id"))
    ...
    print(stocks)
    print("\n")
    return render_template("index.html", new_portfolio=stocks, cash=cash)
...
def buy():
    ...
    if request.method == "POST":
        if lookup(request.form.get("symbol")) is None:
            return apology("invalid stock symbol", 403)ww

        elif float(request.form.get("shares")) < 0:
            return apology("enter positive value", 403)
...
def sell():
    ...
    if request.method == "POST":
        ...
        if request.form.get("symbol") in stocks:
            return apology("Please enter the symbol of a stock you own", 403)
